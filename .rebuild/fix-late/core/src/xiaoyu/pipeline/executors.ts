...
import { CUSTOM_POLICY_VERSION, getCustomAgentConfigurationState, getProjectProviderMode } from "../modelRouting";
...
async function executePreflightStage(ctx: PipelineExecutionContext): Promise<Record<string, unknown>> {
  ...
  if (ctx.run.policyVersion !== CUSTOM_POLICY_VERSION) {
    throw new Error("项目模型配置版本已变化，请重新创建一键生产任务");
  }
  ...
  if (!sourceCount && !String(ctx.project.intro || "").trim()) {
    throw new Error("项目没有小说原文、剧本或故事简介，无法开始一键生产");
  }

  if (!String(ctx.project.imageModel || "").trim() || !String(ctx.project.videoModel || "").trim()) {
    throw new Error("请在项目中选择图片模型和视频模型");
  }

  const agentState = await getCustomAgentConfigurationState();
  if (agentState.missing.length) {
    throw new Error(`还缺少 Agent 模型：${agentState.missing.join("、")}；请在 设置 → Agent 配置 中选择文本模型`);
  }

  const localResources = await assertXiaoyuProductionResources({
    qualityMode: ctx.run.qualityMode,
    episodeCount: ctx.options.episodeCount,
    shotsPerEpisode: ctx.options.shotsPerEpisode,
    enableVoice: ctx.options.enableVoice,
    enableMusic: ctx.options.enableMusic,
  });
  const { ffmpeg, ffprobe } = await ensureMediaRuntime();

  const output = {
    providerMode,
    policyVersion: CUSTOM_POLICY_VERSION,
    qualityMode: ctx.run.qualityMode,
    sourceCount,
    localResources,
    mediaRuntime: { ffmpeg, ffprobe },
    commercialOutput: {
      voice: ctx.options.enableVoice,
      subtitles: ctx.options.enableSubtitles,
      music: ctx.options.enableMusic,
      keepClipAudio: ctx.options.keepClipAudio,
    },
    models: {
      image: String(ctx.project.imageModel),
      video: String(ctx.project.videoModel),
    },
  };

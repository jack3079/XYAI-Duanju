...
/**
 * 小鱼智算中心只是普通可选 Provider。启用它只改变该 Provider 的启用状态，
 * 不再修改 agentUseMode，也不再覆盖任何 Agent 的 vendorId/model/modelName。
 */
...
async function cleanupLegacyAutoBindings(): Promise<void> {
  ...
  // 仅清理旧版自动写入的精确固定别名；其他用户自定义 Provider/模型保持不动。
...
type XiaoyuBootstrapRuntimeState = typeof globalThis & {
  __xiaoyuBootstrapPromise?: Promise<void>;
};
...
async function bootstrapXiaoyuInternal(): Promise<void> {
  ...
  try {
    currentValues = current?.inputValues ? JSON.parse(String(current.inputValues)) : {};
  } catch {
    currentValues = {};
  }
  ...
  const defaultQualityMode = String(
    qualitySetting?.value || currentValues.defaultQualityMode || currentValues.qualityMode || XIAOYU_BRAND.defaultQualityMode,
  );
  ...
  const explicitlyEnabled = String(enabledSetting?.value || "0") === "1";
  ...
  // 只恢复 Provider 的启用状态，不修改任何 Agent/项目模型配置。

...
export function createXiaoyuMaintenanceRouter(): any {
  ...
  router.get("/status", async (_, res) => {
    try {
      ...
      try {
        ...
        integrity = await (u.db as any).raw("PRAGMA integrity_check");
  ...
  router.post("/resources/check", async (req, res) => {
    try {
      res.send(success(await inspectXiaoyuProductionResources({
        qualityMode: String(req.body?.qualityMode || "standard"),
        episodeCount: Number(req.body?.episodeCount || 1),
        shotsPerEpisode: Number(req.body?.shotsPerEpisode || 8),
        enableVoice: req.body?.enableVoice !== false,
        enableMusic: Boolean(req.body?.enableMusic),
      })));
    } catch (exception) { fail(res, exception); }
  });
  ...
  router.post("/restore/prepare", async (req, res) => {
    try {
      if (typeof process.versions?.electron === "undefined") {
        throw new Error("Docker/服务端模式当前支持创建和下载备份，但不支持应用内恢复；请使用持久化 volume 快照恢复，避免运行中的 SQLite 数据被覆盖");
      }
